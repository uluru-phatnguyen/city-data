...
import { createWaterSupplyService, getListWaterSupplyService, uploadWaterSupplyService } from "src/services";
import { useForm } from "react-hook-form";

const columnHelper = createColumnHelper<WaterSupply>();
const columns = [
...
const WaterSupplyPage: React.FC = () => {
  ...
  const listQuery = useQuery({
    queryKey: "getList",
    queryFn: () => getListWaterSupplyService(),
    retry: 0,
  });

  const createMutation = useMutation({
    mutationKey: "onCreate",
    mutationFn: createWaterSupplyService,
    onSuccess: () => {
      listQuery.refetch();
      setShowPopup(false);
  ...
  const importMutation = useMutation({
    mutationKey: "import",
    mutationFn: uploadWaterSupplyService,
    onSuccess: () => {
      listQuery.refetch();
      setShowPopup(false);
      setShowImportPopup(false);
  ...
  return (
    <Container>
        ...
        <div className="p-3">
          <form onSubmit={onSubmit}>
            ...
            <div className="mb-4">
              ...
              <select {...register("quality")} id="quality" name="quality" className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm h-9 pl-3">
                <option value="">--</option>
      ...
      <div className="flex w-full justify-between mb-4 items-center">
        <div className="text-xl font-semibold">Water supply</div>
        <div>
          <button
